    ...
    def _create_sp(self) -> None:
        ...
        for script in self.sp_scripts:
            self.bq_client.create_routine_from_ddl_if_not_exists(
                self.conf.bigquery.indicadores.indicadores_metas[script.name]["sp_name"],
                query=script.content,
                query_parameters=self.conf.bigquery.indicadores.indicadores_metas[script.name]
            )

    def _execute(self, granularity: str) -> None:
        """
        Executes the full load SQL script.
        """
        script_name: str = self.conf.bigquery.script_names.indicadores_metas.endpoint_delta
        script: SQLScript = list(filter(lambda x: x.name == script_name, self.endpoint_scripts))[0]

        self.bq_client.sql(
            script.content,
            query_parameters={
                "sp_merge_indicadores_metas": self.conf.bigquery.indicadores.indicadores_metas.endpoint_delta.sp_merge_indicadores_metas,
                "source_table": self.conf.bigquery.variables.source_target_table[granularity],
                "target_table": self.conf.bigquery.variables.target_table[granularity],
            }
        )
