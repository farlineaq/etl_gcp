...
class BigQueryStage(IBigQueryStage):
    ...
    def _create_sp(self) -> None:
        ...
        for script in self.sp_scripts:
            self.bq_client.create_routine_from_ddl_if_not_exists(
                self.conf.bigquery.indicadores.dim_cadena[script.name]["sp_name"],
                query=script.content,
                query_parameters=self.conf.bigquery.indicadores.dim_cadena[script.name]
            )
    ...
    def _execute(self) -> None:
        ...
        """
        script_name: str = self.conf.bigquery.script_names.dim_cadena.endpoint_carga_inicial
