...
class BigQueryStage(IBigQueryStage):
    ...
    def _create_sp(self) -> None:
        for script in self.sp_scripts:
            self.bq_client.create_routine_from_ddl_if_not_exists(
                self.conf.bigquery.indicadores.transacciones_monitoreadas[script.name]["sp_name"],
                query=script.content,
                query_parameters=self.conf.bigquery.indicadores.transacciones_monitoreadas[script.name]
            )

    def _execute_delta(self, granularity: str) -> None:
        script_name: str = self.conf.bigquery.script_names.transacciones_monitoreadas.endpoint_delta
        script: SQLScript = list(filter(lambda x: x.name == script_name, self.endpoint_scripts))[0]
        self.bq_client.sql(
            script.content,
            query_parameters={
                "sp_delta": self.conf.bigquery.indicadores.transacciones_monitoreadas.endpoint_delta.sp_delta,
    ...
    def _execute_full(self, granularity: str) -> None:
        script_name: str = self.conf.bigquery.script_names.transacciones_monitoreadas.endpoint_carga_inicial
        script: SQLScript = list(filter(lambda x: x.name == script_name, self.endpoint_scripts))[0]
        self.bq_client.sql(
            script.content,
            query_parameters={
                "sp_carga_inicial": self.conf.bigquery.indicadores.transacciones_monitoreadas.endpoint_carga_inicial.sp_carga_inicial,
