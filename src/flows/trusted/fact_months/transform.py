...
class TransformStage(Transform):
    ...
    def merge_query(self):
        self.spark.sql(f"""
            MERGE INTO default.{self.conf.paths.entity_names.fact_months} AS target
    ...
    def optimize_query(self):
        self.spark.sql(f"OPTIMIZE default.{self.conf.paths.entity_names.fact_months}")
        self.spark.sql(f"""
            VACUUM default.{self.conf.paths.entity_names.fact_months} 
    ...
    def register_metrics(self, batch_id: int):
        self.context.count.update(
            get_metrics_from_delta_table(
                self.spark,
                self.conf.paths.fact_months.trusted_data_path
            )
        )
