...
class TransformStage(Transform):
    ...
    def merge_query(self):
        self.spark.sql(f"""
            MERGE INTO default.{self.conf.paths.entity_names.contactabilidad} AS target
    ...
    def optimize_query(self):
        self.spark.sql(f"OPTIMIZE default.{self.conf.paths.entity_names.contactabilidad}")
        self.spark.sql(f"""
            VACUUM default.{self.conf.paths.entity_names.contactabilidad} 
    ...
    def register_metrics(self, batch_id: int):
        self.context.count.update(
            get_metrics_from_delta_table(
                self.spark,
                self.conf.paths.contactabilidad.trusted_data_path
            )
        )
