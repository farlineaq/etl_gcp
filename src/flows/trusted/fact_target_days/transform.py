...
class TransformStage(Transform):
    ...
    def merge_query(self):
        self.spark.sql(f"""
            MERGE INTO default.{self.conf.paths.entity_names.fact_target_days} AS target
    ...
    def optimize_query(self):
        self.spark.sql(f"OPTIMIZE default.{self.conf.paths.entity_names.fact_target_days}")
        self.spark.sql(f"""
            VACUUM default.{self.conf.paths.entity_names.fact_target_days} 
    ...
    def register_metrics(self, batch_id: int):
        self.context.count.update(
            get_metrics_from_delta_table(
                self.spark,
                self.conf.paths.fact_target_days.trusted_data_path
            )
        )
