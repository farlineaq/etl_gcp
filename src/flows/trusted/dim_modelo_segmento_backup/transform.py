...
class TransformStage(Transform):
    ...
    def overwrite_query(self):
        self.spark.sql(f"""
            INSERT OVERWRITE TABLE default.{self.conf.paths.entity_names.dim_modelo_segmento_backup}
            SELECT * FROM global_temp.deduplicated_batch
        """)

    def optimize_query(self):
        self.spark.sql(f"OPTIMIZE default.{self.conf.paths.entity_names.dim_modelo_segmento_backup}")
        self.spark.sql(f"""
            VACUUM default.{self.conf.paths.entity_names.dim_modelo_segmento_backup} 
    ...
    def register_metrics(self, batch_id: int):
        self.context.count.update(
            get_metrics_from_delta_table(
                self.spark,
                self.conf.paths.dim_modelo_segmento_backup.trusted_data_path
            )
        )
