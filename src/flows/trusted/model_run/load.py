...
class LoadStage(Load):
    ...
    def create_delta_table(self):
        return self.spark.sql(f"""
            CREATE TABLE IF NOT EXISTS {self.conf.paths.entity_names.model_run} (
                Modelid INTEGER,
                ModelRunid INTEGER,
                ModelRunDt DATE,
            ...
            USING DELTA
            LOCATION '{self.conf.paths.model_run.trusted_data_path}'
        """)
    ...
    def execute(self, dataframe: DataFrame, transform: Callable[..., None]) -> DataStreamWriter:
        ...
        return (
            ...
            .writeStream
            .format("delta")
            .option("checkpointLocation", f"{self.conf.paths.model_run.trusted_data_path}/_checkpoint")
            .trigger(once=True)
