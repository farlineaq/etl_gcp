...
class TransformStage(Transform):
    ...
    def merge_query(self):
        self.spark.sql(f"""
            MERGE INTO default.{self.conf.paths.entity_names.fact_target_years} AS target
    ...
    def optimize_query(self):
        self.spark.sql(f"OPTIMIZE default.{self.conf.paths.entity_names.fact_target_years}")
        self.spark.sql(f"""
            VACUUM default.{self.conf.paths.entity_names.fact_target_years} 
    ...
    def register_metrics(self, batch_id: int):
        self.context.count.update(
            get_metrics_from_delta_table(
                self.spark,
                self.conf.paths.fact_target_years.trusted_data_path
            )
        )
