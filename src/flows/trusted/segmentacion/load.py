...
class LoadStage(Load):
    ...
    def create_delta_table(self):
        return self.spark.sql(f"""
            CREATE TABLE IF NOT EXISTS {self.conf.paths.entity_names.segmentacion} (
                PartyID INTEGER,
            ...
            USING DELTA
            LOCATION '{self.conf.paths.segmentacion.trusted_data_path}'
        """)
    ...
    def execute(self, dataframe: DataFrame, transform: Callable[..., None]) -> DataStreamWriter:
        ...
        return (
            ...
            .writeStream
            .format("delta")
            .option("checkpointLocation", f"{self.conf.paths.segmentacion.trusted_data_path}/_checkpoint")
            .trigger(once=True)
